...
class Member {
    ...
    borrowItem(item: LibraryItem, days: number): boolean {
        if (!this.canBorrow()) {
    ...
    returnItem(item: LibraryItem, returnDate: Date): number {
...
class Librarian {
    ...
    static libraryName: string = "Ali Ki Library"
